...
export var stopm_impulse = 1000.0


func _on_EnemyDetector_area_entered(area):
	_velocity = calculate_stomp_velocity(_velocity, stopm_impulse)
...
func _physics_process(delta):
	var is_jump_interupted = Input.is_action_just_released("move_up") and _velocity.y < 0
